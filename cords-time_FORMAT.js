...
pipeline.on('data', ({ key, value }) => {
    ...
    outObj[`${value.latitudeE7}:${value.longitudeE7}`] = []
});

pipeline.on('data', ({ key, value }) => {
    // Здесь вы можете обрабатывать каждую запись из массива locations
    outObj[`${value.latitudeE7}:${value.longitudeE7}`].push([value.timestamp, value.altitude])
    if (outObj[`${value.latitudeE7}:${value.longitudeE7}`].length > 1) {
        console.log(outObj[`${value.latitudeE7}:${value.longitudeE7}`])
        repeats+=1
    }
    console.log(value.timestamp, repeats, outObj[`${value.latitudeE7}:${value.longitudeE7}`])
});

pipeline.on('end', () => {
    console.log('Чтение и обработка файла завершены.');
    const writeStream = fs.createWriteStream('large-file.json');
